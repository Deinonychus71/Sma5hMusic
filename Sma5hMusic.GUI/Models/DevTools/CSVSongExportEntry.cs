...
namespace Sma5hMusic.GUI.Models.DevTools
{
    ...
    public class CSVSongExportEntryMap : ClassMap<CSVSongExportEntry>
    {
        public CSVSongExportEntryMap()
        {
            ...
            Map(m => m.DbRoot.IsSelectableMovieEdit).Name("db_root.is_selectable_video_edit").Index(24);
            Map(m => m.DbRoot.IsSelectableOriginal).Name("db_root.is_selectable_original").Index(25);
            Map(m => m.DbRoot.IsDlc).Name("db_root.is_dlc").Index(26);
            Map(m => m.DbRoot.IsPatch).Name("db_root.is_patch").Index(27);
            Map(m => m.DbRoot.DlcUiCharaId).Name("db_root.dlc_ui_chara_id").Index(28);
            Map(m => m.DbRoot.DlcMiiHatMotifId).Name("db_root.dlc_mii_hat_motif_id").Index(29);
            Map(m => m.DbRoot.DlcMiiBodyMotifId).Name("db_root.dlc_mii_body_motif_id").Index(30);
